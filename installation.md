...
Laravel 框架会有一些系统上的要求。当然，这些要求在 [Laravel Homestead](/docs/5.5/homestead) 虚拟机上都已经完全配置好了。所以，非常推荐你使用 Homestead 作为你的本地 Laravel 开发环境。
...
不过有更健壮的本地开发选项可用，比如 [Homestead](/docs/5.5/homestead) 和 [Valet](/docs/5.5/valet)。
...
安装 Laravel 之后， 你需要配置一些权限 。 `storage` 和 `bootstrap/cache` 目录应该允许你的 Web 服务器写入，否则 Laravel 将无法写入。如果你使用 [Homestead](/docs/5.5/homestead) 虚拟机，这些权限应该已经被设置好了。
...
当然如果你使用了 [Homestead](/docs/5.5/homestead) 或者 [Valet](/docs/5.5/valet) 的话， 它会自动的帮你设置好优雅链接。  
...
| 用户名 | 头像 | 职能 | 签名 |
|---|---|---|---|
| [@todayqq](https://github.com/todayqq)  | <img class="avatar-66 rm-style" src="https://avatars0.githubusercontent.com/u/12806696?v=4&s=180">  |  翻译  | 谦虚、自律、胸有成竹、不露城府.  |
